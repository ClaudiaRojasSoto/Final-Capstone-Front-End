...
describe('reservationSlice', () => {
  ...
  it('should handle fetchUserReservations.fulfilled', () => {
    ...
    expect(reservationReducer(initialState, {
      type: fetchUserReservations.fulfilled.type,
  ...
  it('should handle fetchUserReservations.rejected', () => {
    ...
    expect(reservationReducer(initialState, {
      type: fetchUserReservations.rejected.type,
      error: { message: error },
    })).toEqual({
      ...initialState,
      error,
  ...
  it('should handle deleteReservation.pending', () => {
    expect(reservationReducer(initialState, {
      type: deleteReservation.pending.type,
    })).toEqual({
      ...initialState,
      loading: true,
    });
  ...
  it('should handle deleteReservation.fulfilled', () => {
    ...
    const action = { type: deleteReservation.fulfilled.type, payload: 1 };
    const expectedState = {
      ...initialState,
      reservations: [],
      loading: false,
    };
    expect(reservationReducer(startingState, action)).toEqual(expectedState);
  });

  it('should handle deleteReservation.rejected', () => {
    const error = 'Error deleting reservation';
    expect(reservationReducer(initialState, {
      type: deleteReservation.rejected.type,
      error: { message: error },
    })).toEqual({
      ...initialState,
      error,
